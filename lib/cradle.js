...
var sys = require("sys"), http = require("http"), events = require('events'), fs = require("fs"), url = require('url');
...
cradle.Connection = function Connection(/* variable args */) {
	var args = Array.prototype.slice.call(arguments), host, port, remote, auth, options = {};
	...
	//initialize the pool
	this.pool = new (cradle.Pool)(this.host, this.port, this.options);
...
cradle.Connection.prototype.rawRequest = function(method, path, options, data, headers) {
	...
	this.pool.dispatchRequest(method.toUpperCase(), path, headers, function(request) {
	...
	//dispatch to pool
//	request = this.pool.dispatchRequest
...
cradle.Connection.prototype.request = function(method, path, /* [options], [data], [headers] */callback) {
	...
	var options = args.shift() || {}, data = args.shift() || null, headers = cradle.merge({
		host : this.host
	}, args.shift() || {});
...
cradle.Connection.prototype.database = function(name) {
	...
	return {
		...
		get : function(id, rev) {
			var that = this, options = null, args = new (Args)(arguments);

			if (Array.isArray(id)) { // Bulk GET
				this.query('POST', '/_all_docs', {
					include_docs : true
				}, {
					keys : id
				}, function(err, res) {
				...
				this.query('GET', id, options, function(err, res) {
					if (!err)
						that.cache.save(res.id, res.json);
					args.callback(err, res);
				});
			}
		},
		...
		save : function(/* [id], [rev], doc | [doc, ...] */) {
			var args = new (Args)(arguments), array = args.all.slice(0), doc, id, rev;

			if (Array.isArray(args.first)) {
				doc = args.first;
			}
			else {
				doc = array.pop(), id = array.shift(), rev = array.shift();
			}
		...
		merge : function(/* [id], doc */) {
			var args = Array.prototype.slice.call(arguments), callback = args.pop(), doc = args.pop(), id = args.pop() || doc._id;
		...
		put : function(id, doc, callback) {
			...
			this.query('PUT', id, null, doc, function(e, res) {
				if (!e) {
					cache.save(id, cradle.merge({}, doc, {
						_rev : res.rev
					}))
				}
		...
		post : function(doc, callback) {
			...
			this.query('POST', '/', null, doc, function(e, res) {
				if (!e) {
					cache.save(res.id, cradle.merge({}, doc, {
						_rev : res.rev
					}))
				}
				callback && callback(e, res);
		...
		compact : function(design) {
			this.query('POST', '/_compact' + (typeof (design) === 'string' ? '/' + design : ''), Args.last(arguments));
		},
		...
		saveAttachment : function(docOrId, attachmentName, contentType, dataOrStream) {
			...
			that.rawRequest('PUT', pathname, rev, dataOrStream, headers).on('response', function(res) {
				resHeaders = {
...
cradle.Connection.prototype.uuids = function(count, callback) {
	if (typeof (count) === 'function') {
		callback = count, count = null
	}
	this.request('GET', '/_uuids', count ? {
		count : count
	} : {}, callback);
};
